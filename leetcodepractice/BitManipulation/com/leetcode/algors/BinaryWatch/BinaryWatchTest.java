...
public class BinaryWatchTest {
	...
	@Test 
	public void binaryWatchNumber1Test() {
		// given
		int x = 1;
		...
		expected = Arrays.asList(arr);
		
		List<String> actual = sl.readBinaryWatch(x);
		Collections.sort(actual);
		Collections.sort(expected);
		// then
		
		for (String s : actual) {
			Assert.assertEquals(true, expected.contains(s));
		}
		
	}
		
	@Test 
	public void binaryWatchNumber3Test() {
		// given
		int x = 3;
		...
		for (String s : expected) {
			Assert.assertEquals("Missing value " + s, true, actual.contains(s));
		}
